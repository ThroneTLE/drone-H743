...
默认倾转限幅和固件硬上限均为 `+-0.314159 rad`，即 `+-18 deg`，是原 `+-30 deg` 的 60%。从 FLASH 加载的旧参数也会被钳制到该范围。默认的倾转力臂参数为 `0.18 m`。
...
所有入口现在统一按“各自中心脉宽 `+-1000 us`，再受 `500..2500 us` 物理边界”计算安全范围。当前控制器又先把倾转角限制到 `+-18 deg`，所以正常闭环下的理论脉宽范围更窄：alpha `1241..1641 us`，beta `1677..2077 us`。
...
控制器倾转角先被限制到 `+-18 deg`。因为：

$$
\Delta u_{18}=k_s\frac{\pi}{10}=200\,\mu s
$$

所以正常闭环控制的理论脉宽范围是：

$$
u_\alpha\in[1441-200,1441+200]
=[1241,1641],\mu s
$$

$$
u_\beta\in[1877-200,1877+200]
=[1677,2077],\mu s
$$
