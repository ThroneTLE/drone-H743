...
- **舵机 1（上方 / alpha）**：模块旋转补偿后是左右物理轴，控制 roll
- **舵机 2（下方 / beta）**：安装在舵机 1 输出端，是前后物理轴，控制 pitch
...
后续直接测量得到：CG 到桨盘平面的距离 `D = 250 mm`，上方
roll/servo1 舵机轴到桨盘平面 `145 mm`，下方 pitch/servo2 舵机轴到
桨盘平面 `105 mm`。因此：

```text
l_roll  = 250 - 145 = 105 mm = 0.105 m
l_pitch = 250 - 105 = 145 mm = 0.145 m
```

倾转后桨盘位置和推力方向一起绕舵机轴旋转。桨盘到舵机轴的位移与
推力方向共线，其叉乘为零，所以机体姿态力矩使用 CG 到对应舵机轴的
距离，不能直接使用 CG 到桨盘的 `D`。
...
τ_roll  =  l_roll · T · β   =  0.105 · T · β   [N·m]
τ_pitch = −l_pitch · T · α = −0.145 · T · α   [N·m]
τ_yaw   = 0                               （同轴双桨差速独立提供）
```

悬停工况（m = 1.367 kg，T = mg = 13.410 N）：

```text
τ_roll  ≈  1.408 * β   [N·m/rad]  ≈  0.02458 * β   [N·m/deg]
τ_pitch ≈ −1.944 * α   [N·m/rad]  ≈ −0.03394 * α  [N·m/deg]
```
...
| 推力力臂 | pitch = 0.145 m，roll = 0.105 m | 直接测量推导 |
| 舵机映射 | 脉宽 ↔ 倾角 | 精确 |
...
输入: [T_cmd, α_cmd, β_cmd, τ_yaw_cmd]
  ...
  ├─ 力臂力矩:          pitch l=0.145m，roll l=0.105m
