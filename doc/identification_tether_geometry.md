...
桨盘平面 `105 mm`。这些尺寸保留为结构几何记录；当前控制器的有效力臂
按实机调试要求统一设置。
...
```text
l_roll_ctrl  = 0.145 m
l_pitch_ctrl = 0.145 m
```

也就是两轴力矩反解都按 145 mm 有效力臂计算。
...
τ_roll  =  l_roll_ctrl · T · β   =  0.145 · T · β   [N·m]
τ_pitch = −l_pitch_ctrl · T · α = −0.145 · T · α   [N·m]
τ_yaw   = 0                               （同轴双桨差速独立提供）
```
...
```text
τ_roll  ≈  1.944 * β   [N·m/rad]  ≈  0.03394 * β   [N·m/deg]
...
| 推力力臂 | pitch = 0.145 m，roll = 0.145 m | 当前控制器使用值 |
| 舵机映射 | 脉宽 ↔ 倾角 | 精确 |
...
输入: [T_cmd, α_cmd, β_cmd, τ_yaw_cmd]
  ...
  ├─ 力臂力矩:          pitch l=0.145m，roll l=0.145m
