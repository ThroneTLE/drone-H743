...
def test_driver_uses_manual_prad_request_and_fixed_length_dma_receive() -> None:
    source = read("Driver/Src/drv_servo.c")

    assert '"#%03uPRAD!"' in source
    assert "HAL_UART_Receive_DMA" in source
    assert "HAL_UARTEx_ReceiveToIdle_DMA" not in source
...
def test_feedback_bench_is_opt_in_and_step_profile_is_bounded() -> None:
    ...
    assert "APP_SERVO_FB_STEP_MAX_DELTA_US  200U" in source
    assert "APP_SERVO_FB_STEP_MAX_SAMPLES   256U" in source
    assert "APP_SERVO_FB_STEP_MAX_HOLD_MS  5000U" in source
    assert "APP_SERVO_FB_CENTER_US          1500U" in source
    assert "APP_ServoFeedbackBench_ApplyTargets" in source
    assert "APP_SERVO_FB_MODE_STEP_HOLD" in source
    assert "SERVO_STEP ready samples=" in source
...
def test_feedback_bench_commands_and_realistic_move_load_are_wired() -> None:
    ...
    assert 'strcmp(tokens[2], "STEP") == 0' in control
